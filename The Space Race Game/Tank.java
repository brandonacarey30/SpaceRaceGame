...
import ScreenManagement.*;
	...
	public void increaseAngle() {
		if (angle <= 200)// maximum angle restriction
		{
			angle = angle + 3;// increases the angle by 2
			fixTurret();
		}
	}

	public void decreaseAngle() {
		if (angle >= -20) // minimum angle restriction
		{  
			
			angle = angle - 3;// decreases the angle by 3
			fixTurret();
	...
	public void fixTurret() {
		int holder;
		int holder2;
		turretAngleX = Math.round(getTankSprite().getX() + (int) (getTankSprite().getWidth() * .5)+3);
